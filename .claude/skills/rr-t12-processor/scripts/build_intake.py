...
# ===========================================================================
# TAB: RedIQ Audit  (categorization-exception report — audit mode)
# ===========================================================================
_SEV_STYLE = {"high": ("C00000", "FCE4D6", "HIGH"),
              "med":  ("BF8F00", "FFF2CC", "MED"),
              "low":  ("808080", "F2F2F2", "LOW")}


def _egr_opex_noi(st):
    rev = am.REVENUE_CODE_SET
    exp = {c for c, _ in am.EXPENSE_CODES}
    egr = sum(st.code_total(c) for c in rev)
    opex = sum(st.code_total(c) for c in exp)
    return egr, opex, egr - opex


def write_rediq_audit(ws, exceptions, st, prop_name, native_noi=None):
    _set(ws, 1, 1, f"{prop_name} — RedIQ Categorization Audit", font=_f(bold=True, color=NAVY, size=15))
    _set(ws, 2, 1, "Independent re-read of every line in the RedIQ export vs RedIQ's own code. "
         "RedIQ ties at the subtotals — this flags where a LINE's category looks wrong (its errors "
         "flow straight into your model). Fix the flagged codes in RedIQ / your model.",
         font=_f(italic=True, color=GREY, size=9))
    egr, opex, noi = _egr_opex_noi(st)
    win = f"{st.month_labels[0]} – {st.month_labels[-1]}" if st.month_labels else "n/a"
    nh = sum(1 for e in exceptions if e["severity"] == "high")
    nm = sum(1 for e in exceptions if e["severity"] == "med")
    nl = sum(1 for e in exceptions if e["severity"] == "low")
    _set(ws, 4, 1, f"Window: {win}   ·   RedIQ EGR ${egr:,.0f}   ·   OpEx ${opex:,.0f}   ·   NOI ${noi:,.0f}",
         font=_f(bold=True, color=NAVY))
    tie_txt = ""
    if native_noi is not None:
        gap = noi - native_noi
        tie_txt = (f"   ·   Operator T12 NOI ${native_noi:,.0f}   ·   Δ ${gap:,.0f} "
                   + ("✓ ties" if abs(gap) <= max(500, 0.005 * abs(native_noi)) else "⚠ DOES NOT TIE"))
    _set(ws, 5, 1, f"Exceptions: {nh} high · {nm} medium · {nl} low{tie_txt}", font=_f(bold=True, color=NAVY))

    hdr = ["", "Line Item", "RedIQ code", "Suggested", "$ / yr (window)", "NOI effect", "Why"]
    hr = 7
    for c, h in enumerate(hdr, 1):
        _set(ws, hr, c, h, font=_f(bold=True, color=WHITE), fill=_fill(HDR_FILL), align="center", wrap=True)
    r = hr + 1
    if not exceptions:
        _set(ws, r, 1, "No categorization exceptions — every material line agrees with our independent read. ✓",
             font=_f(bold=True, color="548235"))
        r += 1
    for e in exceptions:
        color, fill, tag = _SEV_STYLE[e["severity"]]
        _set(ws, r, 1, tag, font=_f(bold=True, color=WHITE), fill=_fill(color), align="center")
        _set(ws, r, 2, e["line"], font=_f())
        _set(ws, r, 3, f'{e["rediq"]}  ({am.CODE_TO_CATEGORY.get(e["rediq"], "?")})', font=_f(color=GREY))
        _set(ws, r, 4, f'{e["suggested"]}  ({am.CODE_TO_CATEGORY.get(e["suggested"], "?")})',
             font=_f(bold=True, color=NAVY), fill=_fill(fill))
        _set(ws, r, 5, e["annual"], font=_f(), fmt=MONEY, align="right")
        _set(ws, r, 6, e["noi_effect"], font=_f(color=GREY, size=9))
        _set(ws, r, 7, e["reason"], font=_f(size=9))
        r += 1
    for c, w in enumerate([7, 34, 20, 20, 15, 30, 60], 1):
        ws.column_dimensions[get_column_letter(c)].width = w
...
# ===========================================================================
# TAB: RedIQ Categorized  (full line list, RedIQ code vs our read — audit mode)
# ===========================================================================
def write_rediq_categorized(ws, st, exceptions):
    flagged = {e["line"]: e for e in exceptions}
    _set(ws, 1, 1, "RedIQ Categorized — every GL line with RedIQ's code and our independent read",
         font=_f(bold=True, color=NAVY, size=13))
    labels = st.month_labels
    base = ["RedIQ", "Suggested", "⚑", "Category", "Line Item (raw GL)"]
    hr = 3
    for c, h in enumerate(base, 1):
        _set(ws, hr, c, h, font=_f(bold=True, color=WHITE), fill=_fill(HDR_FILL), align="center", wrap=True)
    mc0 = len(base) + 1
    for j, lab in enumerate(labels):
        _set(ws, hr, mc0 + j, lab, font=_f(bold=True, color=WHITE), fill=_fill(HDR_FILL), align="center")
    _set(ws, hr, mc0 + len(labels), "Total", font=_f(bold=True, color=WHITE), fill=_fill(HDR_FILL), align="center")
    r = hr + 1
    # iterate the single stitched file's rows in order (audit mode has exactly one)
    rows = st.files[0].t12.rows if st.files else []
    for row in rows:
        if row.rtype != "line":
            continue
        e = flagged.get(row.name)
        zeb = _fill(ZEBRA) if (r - hr) % 2 == 0 else None
        sev = e["severity"] if e else None
        _set(ws, r, 1, row.code, font=_f(color=GREY), align="center", fill=zeb)
        _set(ws, r, 2, (e["suggested"] if e else row.code),
             font=_f(bold=bool(e), color=(NAVY if e else GREY)), align="center", fill=zeb)
        if e:
            _, fill, tag = _SEV_STYLE[sev]
            _set(ws, r, 3, tag, font=_f(bold=True, color=WHITE), fill=_fill(_SEV_STYLE[sev][0]), align="center")
        else:
            _set(ws, r, 3, "", fill=zeb)
        _set(ws, r, 4, row.section, font=_f(color=GREY, size=9), fill=zeb)
        _set(ws, r, 5, row.name, font=_f(), fill=zeb)
        for j, v in enumerate(row.values):
            _set(ws, r, mc0 + j, (v if v else None), font=_f(), fmt=MONEY, align="right", fill=zeb)
        _set(ws, r, mc0 + len(labels), row.total, font=_f(bold=True), fmt=MONEY, align="right", fill=zeb)
        r += 1
    for c, w in enumerate([10, 12, 5, 22, 34], 1):
        ws.column_dimensions[get_column_letter(c)].width = w
    for j in range(len(labels) + 1):
        ws.column_dimensions[get_column_letter(mc0 + j)].width = 11
...
def build(t12_paths, rr_paths, hd_path=None, prop_name=None, out_path=None,
    ...
    if isinstance(rr_paths, (str, bytes)):
        rr_paths = [rr_paths]
    t12_paths = list(t12_paths)
    # AUDIT MODE: if a RedIQ export is among the statements, audit ITS categorization instead
    # of rebuilding the OS from a native T12. The RedIQ export already carries the model's
    # codes and ties at the subtotals; we re-read each line independently and flag likely
    # miscodes. Any native T12 also passed is kept only to cross-tie NOI.
    rediq_paths = [p for p in t12_paths if il.is_rediq_export(p)]
    native_paths = [p for p in t12_paths if p not in rediq_paths]
    audit_mode = bool(rediq_paths)
    audit_exceptions, audit_native_noi = [], None
    if audit_mode:
        rediq_t12 = il.parse_rediq_export(rediq_paths[0])
        audit_exceptions = il.audit_rediq(rediq_t12)
        _nz = len({r.code for r in rediq_t12.rows if r.rtype == "line" and r.code and any(r.values)})
        _sf = il.StmtFile(il._stmt_label(rediq_paths[0]), rediq_paths[0], rediq_t12,
                          [ym for _, ym in rediq_t12.months],
                          rediq_t12.months[-1][1] if rediq_t12.months else (0, 0), _nz)
        st = il.Stitched([_sf])
        if native_paths:                       # cross-tie NOI to the operator's own T12
            try:
                nst = il.stitch_statements(native_paths)
                rev = il.am.REVENUE_CODE_SET; exp = {c for c, _ in il.am.EXPENSE_CODES}
                audit_native_noi = sum(nst.code_total(c) for c in rev) - sum(nst.code_total(c) for c in exp)
            except Exception:
                audit_native_noi = None
    else:
        st = il.stitch_statements(t12_paths)
    ...
    if audit_mode:
        ws_audit = wb.create_sheet("RedIQ Audit")
        ws_cat = wb.create_sheet("RedIQ Categorized")
        ws_t12 = ws_os = None
    else:
        ws_audit = ws_cat = None
    ...
    if audit_mode:
        write_rediq_audit(ws_audit, audit_exceptions, st, prop_name, native_noi=audit_native_noi)
        write_rediq_categorized(ws_cat, st, audit_exceptions)
    else:
    ...
    if problems:
        ...
        nh = sum(1 for e in audit_exceptions if e["severity"] == "high")
        nm = sum(1 for e in audit_exceptions if e["severity"] == "med")
        nl = sum(1 for e in audit_exceptions if e["severity"] == "low")
        egr, opex, noi = _egr_opex_noi(st)
        tie = ""
        if audit_native_noi is not None:
            gap = noi - audit_native_noi
            tie = (f" RedIQ NOI ${noi:,.0f} vs operator T12 ${audit_native_noi:,.0f} (Δ ${gap:,.0f})"
                   if abs(gap) > max(500, 0.005 * abs(audit_native_noi))
                   else " RedIQ NOI ties operator T12.")
        print(f"AUDIT_OK: {len(audit_exceptions)} categorization exceptions "
              f"({nh} high / {nm} med / {nl} low) over {len(st.monthkeys)} months; "
              f"{len(rr.units)} units, {len(mix)} floor plans.{tie}")
    else:

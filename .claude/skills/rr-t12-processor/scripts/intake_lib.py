...
# ===========================================================================
# RedIQ EXPORT — parse + audit
#   A RedIQ "Actuals / Operating Statement" export already carries RedIQ's OWN
#   standardized code per GL line (the model's categorization). We parse it into
#   a T12 (codes = RedIQ's) and audit each line against our independent read,
#   surfacing likely categorization errors — instead of rebuilding the OS.
# ===========================================================================
def is_rediq_export(path: str) -> bool:
    """True if `path` is a RedIQ standardized export (Overview + a per-line detail
    tab whose header is Code / Category / Line Item), not a native operator T12."""
    try:
    ...
    try:
        ...
        if "Overview" not in names:
            return False
        for sn in wb.sheetnames:
            if sn in ("Overview", "About"):
                continue
            ws = wb[sn]
            for r in range(1, min(ws.max_row, 8) + 1):
                row = [_s(ws.cell(r, c).value).lower() for c in range(1, 4)]
                if row[:3] == ["code", "category", "line item"]:
                    return True
        return False
    finally:
        wb.close()
...
def parse_rediq_export(path: str) -> T12:
    """Parse the RedIQ export's per-line detail tab into a T12 whose line rows
    carry RedIQ's OWN code. Section headers/subtotals (no code) are skipped."""
    wb = openpyxl.load_workbook(path, data_only=True)
    # property name from Overview A1 (falls back to filename in the caller)
    title = ""
    if "Overview" in wb.sheetnames:
        title = _s(wb["Overview"].cell(1, 1).value) or _s(wb["Overview"].cell(1, 2).value)
    det = None
    for sn in wb.sheetnames:
        if sn in ("Overview", "About"):
            continue
        ws = wb[sn]
        for r in range(1, min(ws.max_row, 8) + 1):
            if [_s(ws.cell(r, c).value).lower() for c in range(1, 4)] == ["code", "category", "line item"]:
                det = ws
                hdr_row = r
                break
        if det is not None:
            break
    if det is None:
        raise ValueError("RedIQ export: no Code/Category/Line Item detail tab found")

    # month columns: scan the WHOLE header row for month-like headers (RedIQ detail tabs
    # can span multiple year-blocks separated by annual-summary columns). A 'Monthly' /
    # 'Annual' label row sits just above the header; when present, keep only 'Monthly'
    # columns so annual-Dec columns aren't mistaken for months. Take the MOST RECENT 12
    # (rightmost) so the window matches the Overview's trailing-12 monthly block.
    label_row = hdr_row - 1
    have_labels = any(_s(det.cell(label_row, c).value).lower() in ("monthly", "annual")
                      for c in range(4, det.max_column + 1))
    cand = []
    for c in range(4, det.max_column + 1):
        if have_labels and _s(det.cell(label_row, c).value).lower() != "monthly":
            continue
        v = det.cell(hdr_row, c).value
        if _looks_like_month(v):
            cand.append((c, _month_label(v)))
    cand = cand[-12:]                 # most-recent 12 monthly columns
    month_cols = [c for c, _ in cand]
    months = [m for _, m in cand]

    rows = []
    for r in range(hdr_row + 1, det.max_row + 1):
        code = _s(det.cell(r, 1).value)
        cat  = _s(det.cell(r, 2).value)
        name = _s(det.cell(r, 3).value)
        if not code:                 # section header or subtotal — no code
            if name:
                rows.append(T12Row("header", name, cat))
            continue
        if code not in am.CODE_SECTION:   # unknown/renamed code — keep, treat as expense
            side = "exp"
        else:
            side = "rev" if am.CODE_SECTION[code] in ("rev", "nonop-rev") else "exp"
        vals = [_num(det.cell(r, mc).value) for mc in month_cols]
        # RedIQ's DETAIL tab keeps the RUBS recovery lines (RWS/RT/RF) as NEGATIVE
        # contra-expenses even though its Overview presents them as POSITIVE revenue (the
        # gross-up). Flip the sign for those codes when stored net-negative, so detail-derived
        # totals match the Overview and EGR/OpEx/NOI tie. Scope is exactly the RUBS recovery
        # codes — OI/park/Rentinc keep their Overview sign (a negative OI contra is real).
        if code in ("RWS", "RT", "RF") and sum(vals) < 0:
            vals = [-v for v in vals]
        rows.append(T12Row("line", name, cat, side, "", code, vals, sum(vals)))
    return T12(months=months, rows=rows, sheet_name=det.title, title=title)


# Line-name patterns whose disagreement with RedIQ is a KNOWN, high-value miscode
# (these are the ones we've caught on real deals). Used to rank audit findings.
_AUDIT_CONTENTIOUS = re.compile(
    r"tax\s+(consult|protest|appeal|advis)|lease.?up|rebill|reimburs|boiler|"
    r"workers?.?comp|month.?to.?month|amenity|ground lease|land lease|"
    r"valet|trash|recreational|forced.?place|tenant.?liab|legal.?liab", re.I)


def audit_rediq(t12: T12):
    """Compare RedIQ's code on each line to our INDEPENDENT read and return a
    ranked list of exceptions. Each item:
      {line, category, rediq, suggested, annual, severity, noi_effect, reason}
    Severity: 'high' (side flip rev<->exp, or a negative expense that is really
    income), 'med' (within-side reclass on a known-contentious line), 'low'
    (other judgment-call reclass). We feed RedIQ's own Category as the section
    hint, so our overrides/splitters run fairly and only NAME-driven signals
    (not circular echoes) produce a finding."""
    out = []
    for r in t12.lines():
        rediq = r.code
        # our independent read: name + RedIQ category as section hint (overrides still fire)
        try:
            sugg = am.categorize_t12_line(r.name, r.side, r.section, "")
        except Exception:
            sugg = rediq
        nz = [v for v in r.values if abs(v) > 1e-9]
        neg_expense = (am.CODE_SECTION.get(rediq) == "opex" and nz
                       and sum(v for v in nz) < 0
                       and sum(1 for v in nz if v < 0) / len(nz) >= 0.6
                       and abs(r.total) >= 1000)          # materiality floor
        if sugg == rediq and not neg_expense:
            continue
        if abs(r.total) < 500:        # global materiality floor — nothing sub-$500/yr is worth flagging
            continue
        # suppress immaterial pure judgment-call reclasses (keep all side-flips/neg-expense)
        rsec0 = am.CODE_SECTION.get(rediq, "opex"); ssec0 = am.CODE_SECTION.get(sugg, "opex")
        _same_side = (rsec0 in {"rev", "opex"}) == (ssec0 in {"rev", "opex"}) and rsec0 == ssec0
        if not neg_expense and _same_side and abs(r.total) < 2500:
            continue
        rsec = am.CODE_SECTION.get(rediq, "opex")
        ssec = am.CODE_SECTION.get(sugg, "opex")
        # NOI effect: a rev<->opex flip grosses up EGR & OpEx (NOI-neutral but moves
        # the ratios); an operating<->nonop flip actually MOVES NOI.
        rev_opex = {"rev", "opex"}
        moves_noi = (rsec in rev_opex) != (ssec in rev_opex)
        if neg_expense:
            severity = "high"
            suggested = sugg if ssec == "rev" else "RF"
            reason = ("Net-NEGATIVE expense — a resident recovery/contra booked against expense; "
                      "almost always income (RUBS recovery RF/RWS/RT or Other Income), not a cost.")
            noi_effect = "grosses up EGR & OpEx (NOI-neutral) — but fixes the recovery/ratio"
        else:
            suggested = sugg
            side_flip = (rsec in rev_opex and ssec in rev_opex and rsec != ssec)
            if side_flip or moves_noi:
                severity = "high"
            elif _AUDIT_CONTENTIOUS.search(r.name or ""):
                severity = "med"
            else:
                severity = "low"
            reason = _audit_reason(r.name, rediq, suggested)
            noi_effect = ("MOVES NOI — operating vs non-operating boundary" if moves_noi
                          else ("grosses up EGR & OpEx (NOI-neutral)" if side_flip
                                else "NOI-neutral reclass (shifts a ratio line)"))
        out.append({
            "line": r.name, "category": r.section, "rediq": rediq, "suggested": suggested,
            "annual": r.total, "severity": severity, "noi_effect": noi_effect, "reason": reason,
        })
    rank = {"high": 0, "med": 1, "low": 2}
    out.sort(key=lambda e: (rank[e["severity"]], -abs(e["annual"])))
    return out


def _audit_reason(name, rediq, sugg):
    n = (name or "").lower()
    cat = am.CODE_TO_CATEGORY
    if re.search(r"tax\s+(consult|protest|appeal|advis)", n):
        return "Tax consultant/protest fee is a professional service (G&A), not the ad-valorem tax — leaving it in taxes overstates in-place RE taxes."
    if re.search(r"lease.?up", n):
        return "A one-time lease-up fee is a marketing/lease-up cost (adv), not a recurring management fee."
    if re.search(r"rebill|reimburs", n):
        # Direction matters: a rebill SERVICE/billing fee is the COST to run RUBS (UF); a
        # resident rebill/reimbursement recovered from residents is RUBS REVENUE (RF/RWS/RT).
        if sugg == "UF":
            return "A utility rebill 'service'/'billing fee' is the COST to run the RUBS program (UF), not Other Income — the '…service/fee' wording is the tell."
        if sugg in ("RF", "RWS", "RT"):
            return "A resident rebill/reimbursement recovered from residents is a RUBS recovery (revenue), not a cost."
        return "Rebill/reimbursement line — check whether it is the RUBS program COST (UF) or the resident RECOVERY (RF)."
    if re.search(r"recreational|amenit", n):
        return "Amenity/recreational upkeep is physical R&M (inter/exte), not G&A."
    if re.search(r"workers?.?comp", n):
        return "Workers' comp stays in base Payroll (Pay) in RedIQ's chart, not burden."
    if re.search(r"\btrash\b|valet", n):
        return "Trash hauling files under Contract (cont); valet-trash recovery is RUBS Trash (RT)."
    return f"Our read is '{sugg}' ({cat.get(sugg, sugg)}) vs RedIQ '{rediq}' ({cat.get(rediq, rediq)}) — review."

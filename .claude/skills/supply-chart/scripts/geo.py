...
class Locator:
    ...
    def locate(self, name, address, city, state, zipcode):
        ...
        if self.use_geocoder:
            ...
            cands = []                             # [(dist_to_anchor|None, is_road, ll)]
            for q in queries:
                if q in self.cache:
                    v = self.cache[q]
                else:
                ...
                if not self._plausible(ll):
                    continue
                is_road = len(v) > 2 and v[2] == "highway"
                d = haversine_miles(self.anchor, ll) if self.anchor else None
                cands.append((d, is_road, ll))
            if cands:
                if self.anchor:
                    # With a subject anchor, the CLOSEST plausible hit wins. This
                    # fixes both failure modes: a wrong road-centroid that inflates
                    # distance (a nearer real hit beats it), AND a correct road-
                    # centroid intersection that a farther "place" match would
                    # otherwise override (e.g. "W Bell Rd & AZ-101").
                    cands.sort(key=lambda c: c[0])
                    return cands[0][2], False
                # No anchor: prefer a real point over a road centroid, in query order.
                for d, is_road, ll in cands:
                    if not is_road:
                        return ll, False
                return cands[0][2], False
